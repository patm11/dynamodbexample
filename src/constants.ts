...
const ATTRIBUTE_DEFINITION_DATE: AttributeDefinition = {
  AttributeName: ATTRIBUTE_DATE,
  AttributeType: ATTRIBUTE_TYPE_NUMBER
};
...
const KEY_SCHEMA: KeySchema = [KEY_SCHEMA_ID_ELEMENT, KEY_SCHEMA_DATE_ELEMENT];
...
const BUILD_ATTRIBUTE_MAP = (
  idAttributeValue: AttributeValue,
  dateAttributeValue: AttributeValue
): PutItemInputAttributeMap => {
  return {
...
export const BUILD_PUT_ITEM_INPUT = (id: string, date: Date): PutItemInput => {
  return {
    TableName: TABLE_NAME,
    Item: BUILD_ATTRIBUTE_MAP(
      BUILD_ID_ATTRIBUTE_VALUE(id),
      BUILD_DATE_ATTRIBUTE_VALUE(date)
    )
  };
};
